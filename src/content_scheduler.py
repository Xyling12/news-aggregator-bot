        ...
        if rubric == "video_story":
            try:
                ...
                if vk and vk.enabled:
                    ...
                    # 2. Получаем видео для сторис из Telegram канала
                    from src.media_processor import MediaProcessor
                    import os, json
                    
                    history_path = os.path.normpath(
                        os.path.join(self.config.media_dir, "..", "data", "story_clip_history.json")
                    )
                    os.makedirs(os.path.dirname(history_path), exist_ok=True)
                    try:
                        with open(history_path, "r") as hf:
                    ...
                    mp = MediaProcessor(media_dir=self.config.media_dir)
                    clip_path, target_url = await mp.fetch_telegram_clip(["russia_travel", "russianature", "nature"], exclude_urls=used_urls)
                    ...
                    if clip_path:
                        ...
                        # vid_path will be generated by our story generator on top of the downloaded clip
                        vid_path = await self.story_generator.generate_video_story(clip_path, short_text)
