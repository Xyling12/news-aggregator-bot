...
- НЕ используй эмодзи-цифры (1️⃣ 2️⃣ 3️⃣ и т.п.) — они отображаются некорректно! Используй обычные цифры: 1. 2. 3.
"""
...
1. Начни с эмодзи погоды + "Ижевск, {date}" — первой строкой зацепи читателя
2. 1-2 предложения — что реально ожидать на улице
3. Конкретный совет: что надеть / взять / учесть (не абстрактный, а живой)

ИНТОНАЦИЯ СЕГОДНЯ: {tone}
ЗАПРЕЩЕНО начинать с «ну и погодка» — эта фраза уже приелась читателям.
Максимум 5 строк. Пиши как живой человек с характером, не как сводка Росгидромета.
""" + HUMAN_STYLE

# Rotating voices so daily weather posts don't all sound the same
WEATHER_TONES = [
    "деловая и бодрая: коротко, по фактам, один точный совет",
    "тёплая и заботливая: как будто пишешь другу перед выходом из дома",
    "слегка ироничная: одна лёгкая шутка про погоду, без перегиба",
    "наблюдательная: начни с детали, которую заметит каждый на улице",
    "практичная: сфокусируйся на том, как погода повлияет на планы (дорога, прогулка, дача)",
    "восторженная или сочувствующая — в зависимости от того, хорошая погода или нет",
]
...
СТРОГО ЗАПРЕЩЕНО: выдумывать конкретные даты событий («с 1 декабря», «в марте 2024»), если не уверен — пиши примерно («несколько лет назад», «в начале века»).
...
1. УДИВЛЕНИЕ: начни с «Оказывается...» или «А вы знали, что...»
2. ЦИФРА: факт обязательно с числом («аж 42 года», «целых 150 метров», «более 3 000 человек»)
3. ИСТОРИЯ: короткая история или контекст за фактом
4. МЕСТНАЯ ПРИВЯЗКА: что-то конкретно связанное с Ижевском или Удмуртией
5. ФИНАЛ С ЭМОЦИЕЙ: самый неожиданный факт — оставь читателя с удивлением или гордостью

Нумеруй факты цифрами 1. 2. 3. 4. 5. — НЕ используй эмодзи-цифры (1️⃣ 2️⃣ и т.п.), они отображаются некорректно!
...
НЕ используй Markdown (*жирный*, _курсив_).
СТРОГО ЗАПРЕЩЕНО: выдумывать конкретные даты, числа, сроки («с 1 декабря», «до 15 числа», «с 2024 года») — пиши только общие советы без придуманных дат.
""" + HUMAN_STYLE

PLACE_PROMPT = """Расскажи КОРОТКО про место в Удмуртии: {topic}

ФОРМАТ (сжато, без воды!):
1. КРЮЧОК 📍 (первая строка): ощущение или неожиданный факт — чтобы захотелось туда.
   НЕ начинай с «Представь, что…» и «Знаете, что…» — эти зачины уже приелись. Чередуй: с детали места, с истории, с вопроса «где в Удмуртии…», с личного «есть место, куда…»
2. СУТЬ (1 короткий абзац): чем место особенное + одна «местная» деталь (лучшее время, как добраться, что увидеть)
3. ПРИЗЫВ (1 фраза): мотивация сходить

ЖЁСТКО: максимум 5-6 строк, без длинных историй и легенд. Эмодзи уместно (📍🏛️🕐), без Markdown.
""" + HUMAN_STYLE
...
PHOTO_KEYWORDS_PROMPT = """Придумай 3-5 ключевых слов НА АНГЛИЙСКОМ для поиска стоковой фотографии к этому посту.

КРИТЕРИИ:
- Слова описывают ВИЗУАЛЬНЫЙ контент — что должно быть НА ФОТО
- ИЗБЕГАЙ фото с людьми, если тема НЕ про людей. Для новостей про инфраструктуру,
  погоду, здания, транспорт — ищи ОБЪЕКТЫ и ПЕЙЗАЖИ, а не людей
- Если люди необходимы (праздник, спорт, семья) — добавь слово "russian" к запросу
- НЕ используй абстрактные понятия, только конкретные объекты и сцены
- Для городских тем предпочитай: architecture, building, infrastructure, cityscape
- Верни ТОЛЬКО слова через запятую, без объяснений

Пример: пост про проблемы с электричеством → power lines, electrical grid, city infrastructure, winter
Пример: пост про снегопад → snowfall, winter, snow covered street, frost
Пример: пост про праздник → russian celebration, festive decoration, holiday
Пример: пост про ёжиков → hedgehog, animal, wildlife, nature
...
HOLIDAYS = {
    ...
    # Ноябрь
    (11, 4): "День народного единства и День государственности Удмуртии",
...
class ContentGenerator:
    ...
    async def _find_photo(self, text: str, hint_keywords: Optional[list] = None) -> Optional[str]:
        ...
        try:
            ...
            if photos:
                ...
                topic_wants_people = any(
                    kw.lower() in people_keywords or "russian" in kw.lower()
                    for kw in keywords
                )
                if not topic_wants_people:
                    people_desc_words = {"group of people", "meeting", "team",
                                         "coworkers", "friends", "colleagues"}
                    filtered = [p for p in photos if not any(
                        w in p.get("description", "").lower()
                        for w in people_desc_words
                    )]
                    if filtered:
                        photos = filtered
                        logger.debug(f"Photo filter: removed {len(photos)} people-heavy results")
    ...
    async def _build_weather_post(
        ...
        # Rotate the voice by day-of-year so consecutive days sound different
        tone = WEATHER_TONES[now.timetuple().tm_yday % len(WEATHER_TONES)]
        ...
        text = await self._ask_ai(prompt, temperature=0.7)
        if not text:
    ...
    async def generate_place(self) -> Tuple[Optional[str], Optional[str]]:
        ...
        hint = ["nature landscape forest", "green hills river"]  # neutral, not a foreign city
